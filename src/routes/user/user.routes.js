...
userRouter
	.route('/update-avatar')
	.patch(isAuth, upload.single('avatar'), Updateavatar);

userRouter
	.route('/update-coverimage')
	.patch(isAuth, upload.single('coverImage'), UpdateCoverImage);

userRouter.route('/current-user').get(isAuth, getCurrentUser);
userRouter
	.route('/update-account')
	.patch(isAuth, updateAccountDetails);
userRouter
	.route('/change-password')
	.patch(isAuth, changeCurrentPassword);

userRouter.route('/refresh-token').post(RefreshAcessToken);

userRouter
	.route('/channel/:username')
	.get(isAuth, getProfileInformation);
